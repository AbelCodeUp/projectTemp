...
class DatePickerItem extends Component {
    ...
    renderDatepickerItem(date, index) {
        let className = 
            ...
            '_bxkset disabled' : '_bxkset';
        if(this.props.format == 'd'){
            className = 
            (date < this.props.min || date > this.props.max) ?
            '_bxkset disabled' : '_bxkset';
        }else{
        ...
        let LiText = TimeUtil.convertDate(date, this.props.format);
        let LiTextDay = LiText.substring(LiText.indexOf('月')+1,LiText.indexOf('日'));
        let LiTextMonth = LiText.substring(0,LiText.indexOf('月'));
        return (
            <li
                key={index}
                className={className}>
                { (LiTextDay == new Date().getDate()) && (LiTextMonth == new Date().getMonth()+1 ) ? '今日' : TimeUtil.convertDate(date, this.props.format)}
            </li>
        );
    }
